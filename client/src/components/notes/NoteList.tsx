          ...
          {notes.map((note) => (
            <div
              ...
              className={`p-3.5 rounded-xl border text-left cursor-pointer transition-all duration-200 ${
                selectedNote?.id === note.id
                  ? 'bg-zinc-800/80 border-zinc-500 text-zinc-100 translate-x-1 shadow-sm font-medium'
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {note.videoTimestamp && (
                  <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-dark-bg text-sky-400 border border-dark-border">
                    {note.videoTimestamp}
                  </span>
                )}
              </div>
